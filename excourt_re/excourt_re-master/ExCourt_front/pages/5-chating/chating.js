...
Page({
  ...
  getUrl() {
    const cont = JSON.parse(JSON.stringify(this.data.my_contacts));
    const requests = cont.map((item, i) =>
      new Promise((resolve) => {
        wx.request({
          url: BASE_URL + '/upload/find_profile',
          method: 'POST',
          data: { student_id: item.student_id.toString() + '.png' },
          success: (res) => {
            if (res.statusCode == 200) {
              item.profileurl = res.data.imageUrl;
            }
            resolve();
          },
          fail: (err) => {
