...
Page({
  ...
  loadJoinCourtRequests: function () {
    ...
    wx.request({
      url: BASE_URL + '/teamup/get_records', // 调用 /get_records 接口
      method: 'POST',
      ...
      success: (res) => {
        if (res.data.status === 'success') {
          ...
          const flattenedRecords = res.data.data; 
          // 映射数据为前端需要的格式
  ...
  acceptRequest: function(e) {
    ...
    wx.request({
      url: BASE_URL + '/teamup/accept',  // 发送同意请求的API
      method: 'POST',
      data: { 
  ...
  rejectRequest: function(e) {
    ...
    wx.request({
      url: BASE_URL + '/teamup/refuse',  // 发送拒绝请求的API
      method: 'POST',
      data: { 
        Teamup_court_id: Teamup_court_id, 
        Teamup_requester_id: Teamup_requester_id
