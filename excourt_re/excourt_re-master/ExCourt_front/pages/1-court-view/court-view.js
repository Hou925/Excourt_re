...
Page({
    ...
    updateCourtGrid(date) {
      ...
      //const selectedDate = date.toISOString().split('T')[0]; // 选择的日期，格式为 YYYY-MM-DD
        wx.request({
            url: `${BASE_URL}/student/owned-courts`, // 替换为实际的后端接口地址
            method: 'POST',
            data: {
                ...
                if (res.statusCode === 200) {
                  ...
                  for(let i = 0;i<courts.length;i++){
                    for(let j = 0;j<courtNames.length;j++){
                      if(courtNames[j]==('场地' + courts[i].Court_no))courts[i].courtId=j;
                    }
    ...
    getExchangeablecourts(){
      wx.request({
        url: `${BASE_URL}/student/get_exchangecourt`, // 替换为实际的后端接口地址
        method: 'POST',
        data: {
    ...
    getTeamupablecourts(){
      wx.request({
        url: BASE_URL + '/student/get_teamupcourt', // 替换为实际的后端接口地址
        method: 'POST',
        data: {
            my_id: this.data.student_id,
        },
    ...
    getReceiveablecourts(){
      wx.request({
        url: BASE_URL + '/student/get_offercourt', // 替换为实际的后端接口地址
        method: 'POST',
        data: {
            my_id: this.data.student_id,
        },
